	...
	$("#letras li a").each(function () {
		...
		if ($(this).hasClass('Usado')) {
            ...
            if (!esta) {
    			...
    			gapi.hangout.data.setValue(kSELECTED,elid);
    			var s = "";    			    			
...
function CargaPreguntas(){
	...
	$.getJSON('http://metaversoeducativo.net/sisFH/msicu/php/servicioPreguntasSopa.php', function(data) {
	  	$.each(data.temario, function(indice, valor) {
	  		...
	  		MuestraPregunta(indice);
	  		
	  	});
	});
}

function MuestraPregunta(indice){
	$("#ColumnaDos").append((indice+1)+".- ");
	  		$("#ColumnaDos").append(preguntas[indice] + "<span id='pre"+indice+"' style=\"visibility:hidden\">"+" R.-"+resultados[indice]+"</span>");
	  		$("#ColumnaDos").append("<br/>");
}
	...
	if(kSTATE == kCONTESTADAS){
		...
		if(f){
			...
			for(var i=0; i < ss.length; i++){		
				...
				$('#pre' + ind).css("visibility","visible");
